...
const buildPipelineResult = (result41, result54) => {
  if (!result41 && !result54) return null;

  if (result41 && !result54) {
    return {
      ...result41,
  ...
  return {
    success: result41.success && result54.success,
    model: "gpt-4.1 + gpt-5.4 pipeline",
    latencyMs: Math.max(
      result41.latencyMs || 0,
      result54.latencyMs || 0
    ),
    usage,
    parsed: {
      reports,
...
export default function App() {
  ...
  async function handleAnalyze() {
    try {
      ...
      const pipelineResult = apiResults.find(
        (item) =>
          item.model &&
          item.model
            .toLowerCase()
      ...
      const result41 = apiResults.find(
        (item) => item.model === "gpt-4.1"
      );
      const result54 = apiResults.find(
        (item) => item.model === "gpt-5.4"
      );
      const mergedResult =
        pipelineResult ||
        (result41 && result54
          ? buildPipelineResult(result41, result54)
          : null);

      setResults(
        mergedResult ? [mergedResult] : apiResults
      );
    } catch (error) {
      console.error(error);
